    ...
    for row_number, seats in value.items():
        for seat_number, seat in seats:
            if(seat == 1):
                ...
                cursor.execute('INSERT INTO Billett(StykkeID, Forestillingsdato, Stolnummer, Radnummer, OmraadeNavn, SalNavn, Billettype, Pris) VALUES(?, ?, ?, ?, ?, ?, ?, ?);', (stykkeID, date, seat_number, row_number, section, 'Gamle scene', 'Ordinær', 350))
                bilettID = cursor.execute('SELECT BillettID FROM Billett WHERE StykkeID = ? AND Forestillingsdato = ? AND Stolnummer = ? AND Radnummer = ? AND OmraadeNavn = ? AND SalNavn = ? AND Billettype = ? AND Pris = ?;', (stykkeID, date, seat_number, row_number, section, 'Gamle scene', 'Ordinær', 350)).fetchone()[0]
